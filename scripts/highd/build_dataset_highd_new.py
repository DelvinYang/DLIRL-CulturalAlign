"""
Build trajectory-level dataset for the HighD dataset.
...
from datasets.highd.common import State
from datasets.highd.read_data_highd import DataReaderHighD
from datasets.highd.scenariohighd import ScenarioHighD, Vehicle

DEFAULT_DATA_PATH = "/path/to/datasets/highD"
DEFAULT_PREFIXES = [f"{idx:02d}" for idx in range(1, 61)]
...
def build_dataset(
    ...
    for prefix_number in prefixes:
        ...
        try:
            data_reader = DataReaderHighD(prefix_number, data_path)
            scenario = ScenarioHighD(data_reader)
        except Exception as exc:  # noqa
