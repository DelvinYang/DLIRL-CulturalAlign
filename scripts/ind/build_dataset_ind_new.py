"""
Build trajectory-level dataset for the inD dataset.
...
from datasets.ind.common import State, dir_names, eight_dirs
from datasets.ind.read_data_inD import DataReaderInD
from datasets.ind.scenarioind import ScenarioInD, Vehicle

DEFAULT_DATA_PATH = "/path/to/datasets/inD"
DEFAULT_PREFIXES = [f"{idx:02d}" for idx in range(1, 33)]
...
def extract_frame_state(
    ...
    ego_vehicle: Vehicle,
    ego_frenet_system: FrenetSystem,
    frame_num: int,
    max_radius: float,
) -> torch.Tensor | None:
...
def collect_trajectories(
    ...
    for ego_id in tqdm(scenario.id_list, desc="Collecting trajectories"):
        ...
        try:
            ...
            logger.debug("skip ego {} due to {}", ego_id, exc)
            continue
        ...
        for frame_num in range(start_f, end_f + 1):
            try:
                state = extract_frame_state(
                    ...
                    ego_frenet_system=ego_frenet_system,
                    frame_num=frame_num,
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Root directory that contains the inD data folders.",
    )
    parser.add_argument(
        "--prefix",
        nargs="*",
